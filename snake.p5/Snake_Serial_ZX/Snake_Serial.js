...
function preload() {
  backgroundImage = loadImage('assets/bgimage.png');
...
function setup() {
  ...
  // Set colors for snake, apple, and background
...
function draw() {
  ...
  if(snake.dead) {
    if(!gameOverSound.isPlaying()) {
      bgmSound.stop();
      if(!gameOverSound.playedOnce) {
        gameOverSound.play();
        gameOverSound.playedOnce = true;
      }
    }

  }

  // Adjust the background music speed based on the snake speed
  bgmSound.rate(initialSpeedInterval/snake.speedInterval);
...
function resetGame() {
  if (snake.dead) {
    ...
    gameOverSound.playedOnce = false;
  }
}
